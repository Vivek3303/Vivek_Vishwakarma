...
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 2))
TEXT = os.environ.get("WORDMARK_TEXT", "VIVEK")
...
# Custom Palette Injected
BG = "#161b22"           # Dark GitHub Background
BG2 = "#0d1117"          # Darker inset
FRAME = "#1E3156"        # Custom Deep Blue
TITLE_TEXT = "#FDD6CD"   # Custom Peach
INK = "#B0CBE6"          # Custom Light Blue
...
def build_shell():
    ...
    font_size = MASK_H
    for _ in range(40):
        # Fallback to default PIL font if Futura is not found on your specific OS
        try:
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">vivek@mumbai: ~$ ./wordmark.sh --3d</text>')
